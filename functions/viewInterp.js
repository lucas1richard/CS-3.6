    ...
    chrome.storage.sync.get({userInfo:false}, function(item) {
      ...
      var emailSel = (function(){
        ...
        for(var i=0; i<types.length; i++) {
          var opt = document.createElement("option");
              opt.value = types[i].val;
              opt.innerText = types[i].txt;
            if(types[i].disabled) opt.setAttribute("disabled",true);
            emailSel.appendChild(opt);
        }
  
        emailSel.className = "form-control input-sm";
        emailSel.addEventListener("change", newMail);
        return emailSel;
      })();
      ...
      function generateIssuanceLetter() {
        ...
        var addressFull = (function() {
          ...
          function exists(str) {
            if(str.length > 0 && str.toLowerCase() != "none") return true;
            return false;
          }
